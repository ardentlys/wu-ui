<template>
	...
	>
		<view @touchstart="touchstart">
			...
			>
				<view 
					class="wu-popup__content" 
					:style="[contentStyle]" 
					:class="[popupClass]" 
					@click="clear"
				>
					<wu-status-bar v-if="safeAreaInsetTop"></wu-status-bar>
					<slot />
					<wu-safe-bottom v-if="safeAreaInsetBottom"></wu-safe-bottom>
					<view
						v-if="closeable"
						@tap.stop="close"
						class="wu-popup__content__close"
						:class="['wu-popup__content__close--' + closeIconPos]"
						hover-class="wu-popup__content__close--hover"
						hover-stay-time="150"
					>
						<wu-icon
							name="close"
	...
	*/
	export default {
		...
		computed: {
			...
			bg() {
				if (this.bgColor === '' || this.bgColor === 'none' || this.$w.getPx(this.round)>0) {
					return 'transparent'
				}
				return this.bgColor
			},
			contentStyle() {
				...
				if(this.round) {
					const value = this.$w.addUnit(this.round)
		...
		methods: {
			...
			open(direction) {
				...
				if (!this.config[direction]) {
					return this.$w.error(`缺少类型：${direction}`);
				}
