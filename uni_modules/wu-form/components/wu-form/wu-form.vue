...
<script>
	...
	import Schema from "@/uni_modules/wu-ui-tools/libs/util/async-validator.js";
	...
	export default {
		...
		watch: {
			...
			// 监听属性的变化，通知子组件wu-form-item重新获取信息
			propsChange(n) {
			...
			model: {
				...
				handler(n) {
					if (!this.originalModel) {
						this.originalModel = this.$w.deepClone(n);
					}
				},
			},
		},
		...
		methods: {
			...
			setRules(rules) {
				...
				if (process.env.NODE_ENV === 'development' && Object.keys(this.model).length === 0) {
					this.$w.error('设置rules，model必须设置！如果已经设置，请刷新页面。');
					return;
				};
			...
			resetModel(obj) {
				...
				this.children.map((child) => {
					...
					const value = this.$w.getProperty(this.originalModel, prop);
					this.$w.setProperty(this.model, prop, value);
				});
			},
			// 清空校验结果
			clearValidate(props) {
				...
				this.$nextTick(() => {
					...
					this.children.map((child) => {
						...
						if (value.includes(child.prop)) {
							...
							const propertyVal = this.$w.getProperty(
								this.model,
								child.prop
							);
							// 属性链数组
			...
			validate(callback) {
				...
				if (process.env.NODE_ENV === 'development' && Object.keys(this.formRules).length === 0) {
					this.$w.error('未设置rules，请看文档说明！如果已经设置，请刷新页面。');
					return;
				}
					...
					this.$nextTick(() => {
						...
						this.validateField(formItemProps, (errors) => {
							if(errors.length) {
								...
								this.errorType === 'toast' && this.$w.toast(errors[0].message)
								reject(errors)
							} else {
								resolve(true)
							}
