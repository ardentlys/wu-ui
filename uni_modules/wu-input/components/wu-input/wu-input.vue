<template>
	<view class="wu-input" :class="inputClass" :style="[wrapperStyle]">
		<view class="wu-input__content">
			<view class="wu-input__content__prefix-icon">
				<slot name="prefix">
					<wu-icon v-if="prefixIcon" :name="prefixIcon" size="18" :customStyle="prefixIconStyle"></wu-icon>
				</slot>
			</view>
			<view class="wu-input__content__field-wrapper" @tap="clickHandler">
				<!-- 根据uni-app的input组件文档，H5和APP中只要声明了password参数(无论true还是false)，type均失效，此时
					为了防止type=number时，又存在password属性，type无效，此时需要设置password为undefined
				 -->
				<input class="wu-input__content__field-wrapper__field" :style="[inputStyle]" :type="type" :focus="focus"
			...
			<view class="wu-input__content__clear" v-if="isShowClear" @tap="onClear">
				<wu-icon name="close" size="11" color="#ffffff" customStyle="line-height: 12px"></wu-icon>
			</view>
			
			<view class="wu-input__content__subfix-icon">
				<slot name="suffix">
					<wu-icon v-if="suffixIcon" :name="suffixIcon" size="18" :customStyle="suffixIconStyle"></wu-icon>
				</slot>
			</view>
		</view>
	</view>
...
<script>
	 ...
	 * @tutorial https://wu.geeks.ink/components/input.html
	 ...
	 * @example <wu-input v-model="value" :password="true" suffix-icon="lock-fill" />
	 */
	export default {
		name: "wu-input",
		...
		computed: {
			...
			inputClass() {
				...
				border === "surround" &&
					(classes = classes.concat(["wu-border", "wu-input--radius"]));
