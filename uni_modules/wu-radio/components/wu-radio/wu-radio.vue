<template>
	<view class="wu-radio" @tap.stop="wrapperClickHandler" :style="[radioStyle]"
		:class="[`wu-radio-label--${parentData.iconPlacement}`, parentData.borderBottom && parentData.placement === 'column' && 'wu-border-bottom']">
		<view class="wu-radio__icon-wrap" @tap.stop="iconClickHandler" :class="iconClasses" :style="[iconWrapStyle]">
			<slot name="icon">
				<wu-icon class="wu-radio__icon-wrap__icon" name="checkbox-mark" :size="elIconSize"
					:color="elIconColor" />
			</slot>
		</view>
		<slot>
			<text class="wu-radio__text" @tap.stop="labelClickHandler" :style="{
...
<script>
	 ...
	 * @example <wu-radio :labelDisabled="false">门掩黄昏，无计留春住</wu-radio>
	 */
	export default {
		name: "wu-radio",
		emits: ['change'],
		...
		methods: {
			...
			emitEvent() {
				// wu-radio的checked不为true时(意味着未选中)，才发出事件，避免多次点击触发事件
				if (!this.checked) {
...
<style lang="scss" scoped>
	...
	$wu-radio-wrap-margin-right: 6px !default;
	$wu-radio-wrap-font-size: 20px !default;
	$wu-radio-wrap-border-width: 1px !default;
	$wu-radio-wrap-border-color: #c8c9cc !default;
	$wu-radio-line-height: 0 !default;
	$wu-radio-circle-border-radius: 100% !default;
	$wu-radio-square-border-radius: 3px !default;
	$wu-radio-checked-color: #fff !default;
	$wu-radio-checked-background-color: red !default;
	$wu-radio-checked-border-color: #2979ff !default;
	$wu-radio-disabled-background-color: #ebedf0 !default;
	$wu-radio-disabled--checked-color: #c8c9cc !default;
	$wu-radio-label-margin-left: 5px !default;
	$wu-radio-label-margin-right: 12px !default;
	$wu-radio-label-color: $wu-content-color !default;
	$wu-radio-label-font-size: 15px !default;
	$wu-radio-label-disabled-color: #c8c9cc !default;
	...
	.wu-radio {
		...
		&__icon-wrap {
			...
			color: transparent;
			text-align: center;
			margin-right: $wu-radio-wrap-margin-right;
			font-size: $wu-radio-wrap-font-size;
			border-width: $wu-radio-wrap-border-width;
			border-color: $wu-radio-wrap-border-color;
			border-style: solid;

			/* #ifdef MP-TOUTIAO */
			// 头条小程序兼容性问题，需要设置行高为0，否则图标偏下
			&__icon {
				line-height: $wu-radio-line-height;
			}

			/* #endif */

			&--circle {
				border-radius: $wu-radio-circle-border-radius;
			}

			&--square {
				border-radius: $wu-radio-square-border-radius;
			}

			&--checked {
				color: $wu-radio-checked-color;
				background-color: $wu-radio-checked-background-color;
				border-color: $wu-radio-checked-border-color;
			}

			&--disabled {
				background-color: $wu-radio-disabled-background-color !important;
			}

			&--disabled--checked {
				color: $wu-radio-disabled--checked-color !important;
			}
		}
		...
		&__label {
			...
			/* #endif */
			margin-left: $wu-radio-label-margin-left;
			margin-right: $wu-radio-label-margin-right;
			color: $wu-radio-label-color;
			font-size: $wu-radio-label-font-size;

			&--disabled {
				color: $wu-radio-label-disabled-color;
			}
		}
	}
</style>