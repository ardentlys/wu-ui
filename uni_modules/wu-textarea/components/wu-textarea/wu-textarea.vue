<template>
	<view class="wu-textarea" :class="textareaClass" :style="[textareaStyle]">
		<textarea class="wu-textarea__field" :value="innerValue" :style="{ height: $w.addUnit(height) }"
			:placeholder="placeholder" :placeholder-style="$w.addStyle(placeholderStyle, 'string')"
...
<script>
	 ...
	 * @tutorial https://wu.geeks.ink/components/textarea.html
	 *
	 * @property {String | Number} 		value					输入框的内容
	 * @property {String | Number} 		modelValue				输入框的内容
	 ...
	 * @example <wu-textarea v-model="value1" placeholder="请输入内容" ></wu-textarea>
	 */
	export default {
		name: "wu-textarea",
		...
		computed: {
			...
			textareaClass() {
				...
				border === "surround" &&
					(classes = classes.concat(["wu-border", "wu-textarea--radius"]));
				border === "bottom" &&
